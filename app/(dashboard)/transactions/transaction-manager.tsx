...
const inputClass = "mt-2 w-full rounded-xl border border-slate-300 bg-white px-3.5 py-3 text-slate-950 outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 disabled:opacity-60 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:ring-emerald-950";
...
export function TransactionManager({ currency, initialOpen }: { currency: string; initialOpen: boolean }) {
  ...
  return (
    <section>
      ...
      <section aria-labelledby="filters-heading" className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><h2 id="filters-heading" className="font-bold">Filters</h2><button type="button" onClick={clearFilters} className="text-sm font-bold text-emerald-600">Clear all</button></div><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-6"><Field label="Search" htmlFor="search"><input id="search" type="search" placeholder="Description or note" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} className={inputClass} /></Field><Field label="Type" htmlFor="filter-type"><select id="filter-type" value={type} onChange={(event) => { setType(event.target.value); setCategoryId(""); setPage(1); }} className={inputClass}><option value="">All types</option><option value="INCOME">Income</option><option value="EXPENSE">Expense</option></select></Field><Field label="Category" htmlFor="filter-category"><select id="filter-category" value={categoryId} onChange={(event) => { setCategoryId(event.target.value); setPage(1); }} className={inputClass}><option value="">All categories</option>{visibleFilterCategories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></Field><Field label="Month" htmlFor="filter-month"><input id="filter-month" type="month" value={month} onChange={(event) => { setMonth(event.target.value); setPage(1); }} className={inputClass} /></Field><Field label="From date" htmlFor="filter-from"><input id="filter-from" type="date" value={dateFrom} disabled={Boolean(month)} onChange={(event) => { setDateFrom(event.target.value); setPage(1); }} className={inputClass} /></Field><Field label="To date" htmlFor="filter-to"><input id="filter-to" type="date" value={dateTo} disabled={Boolean(month)} onChange={(event) => { setDateTo(event.target.value); setPage(1); }} className={inputClass} /></Field></div></section>
      ...
      {!loading && pagination.total > 0 && <nav aria-label="Transaction pages" className="mt-5 flex items-center justify-between gap-4"><p className="text-sm text-slate-500">Page {pagination.page} of {pagination.totalPages} · {pagination.total} records</p><div className="flex gap-2"><button type="button" disabled={page <= 1} onClick={() => setPage((current) => current - 1)} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-bold disabled:opacity-40 dark:border-slate-700">Previous</button><button type="button" disabled={page >= pagination.totalPages} onClick={() => setPage((current) => current + 1)} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-bold disabled:opacity-40 dark:border-slate-700">Next</button></div></nav>}
    </section>
  );
}

function Field({ label, htmlFor, children }: { label: string; htmlFor: string; children: React.ReactNode }) { return <label htmlFor={htmlFor} className="block text-sm font-semibold text-slate-700 dark:text-slate-200">{label}{children}</label>; }
