  ...
  return (
    <div className="min-h-screen bg-white text-slate-950 dark:bg-slate-950 dark:text-white">
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur dark:border-slate-800 dark:bg-slate-950/90">
        <nav aria-label="Main navigation" className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-3 py-3 sm:px-6 sm:py-4 lg:px-8">
          <Logo />
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
            <ThemeToggle />
            <Link href="/login" className="rounded-full px-2.5 py-2 text-sm font-bold hover:text-emerald-600 sm:px-4">Sign in</Link>
            <Link href="/register" className="rounded-full bg-emerald-500 px-3 py-2.5 text-sm font-bold text-slate-950 transition hover:bg-emerald-400 sm:px-4"><span className="sm:hidden">Start</span><span className="hidden sm:inline">Get started</span></Link>
          </div>
        </nav>
      </header>
      ...
      <main id="main">
        <section className="relative isolate overflow-hidden border-b border-slate-200 dark:border-slate-800">
          ...
          <div className="mx-auto grid max-w-7xl gap-12 px-4 py-14 sm:px-6 sm:py-24 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-8 lg:py-28">
            <div>
              ...
              <h1 className="mt-6 max-w-3xl text-4xl font-black leading-[1.05] tracking-tight sm:mt-7 sm:text-6xl lg:text-7xl">Know where your money goes. <span className="text-emerald-500">Plan what comes next.</span></h1>
              <p className="mt-6 max-w-2xl text-base leading-7 text-slate-600 dark:text-slate-300 sm:mt-7 sm:text-lg sm:leading-8">Record income and expenses, set monthly category budgets, and understand six-month trends from one responsive dashboard.</p>
        ...
        <section id="features" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2">
            {features.map(([title, copy], index) => <article key={title} className="rounded-3xl border border-slate-200 p-5 dark:border-slate-800 dark:bg-slate-900/50 sm:p-7"><span className="text-sm font-black text-emerald-600">0{index + 1}</span><h3 className="mt-4 text-xl font-bold">{title}</h3><p className="mt-3 leading-7 text-slate-600 dark:text-slate-300">{copy}</p></article>)}
          </div>
        </section>

        <section id="how-it-works" className="bg-slate-50 py-16 dark:bg-slate-900/60 sm:py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 className="text-3xl font-black tracking-tight sm:text-4xl">From entry to insight in three steps</h2>
            <ol className="mt-10 grid gap-6 md:grid-cols-3">
              {[['Record', 'Add income and expenses with a date and category.'], ['Plan', 'Set a monthly limit for each expense category.'], ['Review', 'Use dashboard and analytics views to understand the result.']].map(([title, copy], i) => <li key={title} className="rounded-3xl bg-white p-5 shadow-sm dark:bg-slate-950 sm:p-7"><span className="grid h-10 w-10 place-items-center rounded-full bg-emerald-500 font-black text-slate-950">{i + 1}</span><h3 className="mt-5 text-xl font-bold">{title}</h3><p className="mt-3 text-slate-600 dark:text-slate-300">{copy}</p></li>)}
            </ol>
          </div>
        </section>

        <section id="security" className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-2 lg:items-center lg:px-8">
...
function PreviewCard({ label, value, className }: { label: string; value: string; className: string }) {
  return <div className="min-w-0 rounded-2xl bg-slate-50 p-3 dark:bg-slate-800 sm:p-4"><p className="truncate text-[11px] font-semibold text-slate-500 dark:text-slate-400 sm:text-xs">{label}</p><p className={`mt-2 truncate text-base font-black sm:text-xl ${className}`}>{value}</p></div>;
}
